...
json_name = "llava_test_prompt1_addition"

cur_class = image_files[0][-6:-4]
sections = []
section_start = 0

for i, file in enumerate(image_files):
    if cur_class != file[-6:-4]:
        if (cur_class != "00"):
            first = section_start-frames_before
            last = i+frames_after

            if (first >= 0) and (last < len(image_files)):
                sections.append(image_files[first:last])
        
        section_start = i
        cur_class = file[-6:-4]

for section in sections:
    print(f"{section[frames_before][-6:-4]}: {len(section)-frames_before-frames_after}")

for section_files in tqdm(sections):
    for center_frame in tqdm(range(frames_before, len(section_files) - frames_after, interval)):
        first_frame = center_frame - frames_before
        last_frame = center_frame + frames_after

        start_name = os.path.basename(section_files[first_frame])
        end_name = os.path.basename(section_files[last_frame])

        if start_name[:11] != end_name[:11]:
            continue
        
        center_name = os.path.basename(section_files[center_frame])
